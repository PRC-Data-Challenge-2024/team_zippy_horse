...
!https://www.flugzeuginfo.net/acdata_php/acdata_7772_en.php
   ...
   OPen(51,file= &
 'git/team_zippy_horse_v4_d3e32e20-d955-450b-a81a-85e9dad54a89.csv')
     write(51,*)'flight_id,tow'
      call sloopflights
  ...
  ,Jairline,NPS,IS  &
  ,climbdataca  &
  ,lpr,nairlinex,massresult5,clrout,nclrout,nfcmax,nfsmax,flightairdist &
 ...
 !      print*,' JDAYJ0',JDAYJ0
  ...
  Lflightready=.false.
  open(77,file='flighreadS7',form='unformatted')
 ...
 open(77,file='flighreadS7',form='unformatted',action='read')
...
!  print*,' massresult5(1),FLLISTS%tow',massresult5(1),FLLISTS%tow
     
   K=1
      if(imassvalid(K).gt.0) then
   Massresult5(6)=massresult5(K)
    else
    Massresult5(6)=0.3*PMTOM(IPS)+0.7*POEM(IPS)
    end if
   
   
  
   PRint*,'IPS,jairline,ibestm(Ips,Jairline),Massresult5(6)', &
   IPS,jairline,ibestm(Ips,Jairline),Massresult5(6)
...
! also computes flightairdist
   ...
   write(51,*) FLLISTS%flight_id, ',', massresult5(6)
...
!!!!???   if(Nwr50.gt.1000) stop
  ...
  ,' FLIGHT(1)%flight_id',FLIGHT(1)%flight_id,' Nwr50=',Nwr50
   ...
   print*,'number of  missing flights, J=', J
    ...
    subroutine M1_MassfromLF(NW,IPS,IDAY) ! -> massresult5(1) and flightadd(NW)%massfrom(1)
           USE mo_directories, ONLY: FLIGHT, flightadd ,massxfromLF,meanLFd &
           ,imassvalid,lpr,massresult5,meanerrorac,Jairline,NPS
  IMPLICIT NONE
 ...
 !        end if
        ...
        massresult5(IMETHOD)=massresult   
 ...
 ! includes airport coordinates at beginning and end of trajectory
       ...
       I2=0
        ...
        flight(1)%vertical_rate=0.
         print*,'test: FLLISTS%ITWdep,FLIGHTadd(1)%ITW-',FLLISTS%ITWdep,FLIGHTadd(1)%ITW
...
! MassfromDO from design optimum (Ian Poll,2.10.2024),see SMassfromMach
       ...
       ,NPS,massresult5,imassvalid,meanerrorac,IFLMAX
...
!      ,massCLO,((massCLO-flightadd(1)%massw(2))/flightadd(1)%massw(2))
       ...
       subroutine M1_LOADF_wind(NW,IPS,IDAY,TOM)
        ...
        ,flight,imassvalid,flightairdist
  ...
  real RT,fexp,qlm,TAS
       ...
       RT=RT+idt*(flightadd(I-1)%TASW+flightadd(I)%TASW)*0.5      
       end do
 ...
 !  
       ...
       subroutine M4_climbdata(NW,IPS)
         ...
         ,imassvalid,climbdataca,lpr,NPS,massresult5,clrout,nclrout,meanerrorac
  USE mo_cocip_parameters, ONLY: LCVCC
  IMPLICIT NONE
  INTEGER,INTENT(IN):: NW,IPS
  INCLUDE "CPARA.inc"
    ...
    FLIGHTadd(NW)%massfrom(IMETHOD)=FLIGHTadd(NW)%massfrom(IMETHOD)/(1.-meanerrorac(IMETHOD,IPS,Jairline))
     imassvalid(IMETHOD)=0
      ...
      nclrout=0
               ...
               massresult5(IMETHOD)=FLIGHTadd(NW)%massfrom(IMETHOD)

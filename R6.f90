! R5 performs the tow estimates for the final submission data (without tow input)
! using the method as tested in R5.


! makes use of the PS model data, the airport data and 
...
MODULE mo_directories ! many code details
  ...
  IMPLICIT NONE
     ...
     INTEGER,PARAMETER:: NWX=100002
     INTEGER,PARAMETER::nfsmax=158149 
...
PROGRAM main
  ...
  ,climbdataca  &
  ,lpr,nairlinex &
  ,clrmintyp,clrmaxtyp,ibestm,nfsmax
 ...
 Print*,' ALLOCATE: climbdataca(NPS,nairlinex),stat,NPS=',istat,NPS,nairlinex  
   ...
   timetot=0.
      ...
      open(5,file='Results2/ibestm.txt',action='read')
       ...
       close(5)
      open(5,file='git/final_submisson.csv',action='read')
      read(5,*)
      if(LPR) print*,' nfsmax',nfsmax  
      do I=1,nfsmax ! in R6 R5: nfs!!!
      read(5,*) ILINE,massxfromLF(I)
!      print*,' ILINE,massxfromLF(I)',ILINE,massxfromLF(I)
       end do
      close(5)
...
!https://www.flugzeuginfo.net/acdata_php/acdata_7772_en.php
  ...
  ,Jairline,NPS,IS  &
  ,climbdataca  &
  ,lpr,nairlinex,massresult6,clrout,nclrout,nfsmax,flightairdist &
 ...
 !      print*,' JDAYJ0',JDAYJ0
  ...
  Lflightready=.false.
  open(77,file='flighreadS',form='unformatted')
 ...
 open(49,file='oemerrors.txt')
      ...
      , 'itime_sincestart/s ' ,'duration/s ','flown_distance/nml ' &
        ...
        NFOUNDS=0
        nairlinemax=0
   open(51,file='F:\FLLISTSfinal\' &
        //'team_zippy_horse_v2_d3e32e20-d955-450b-a81a-85e9dad54a89.csv')
        write(51,*)'flight_id, tow/kg'
    
 LOOPday: do iday=1,365!271,365!365!262!262!262!1,2!5!1,1!1,1!2
 
 open(77,file='flighreadS',form='unformatted',action='read')
  ...
  close(77)
   ...
   print*,' open:','F:\FLLISTSfinal\'//'outmassS_'//cdate//'.txt'
            
  
  open(50,file='F:\FLLISTSfinal\'//'outmassS_'//cdate//'.txt')
      write(50,*) 'flight_id ','ICAO ','IS ','IPS ','Jairline ' &
 ...
 ,'clr/(m/s) ','nclrsteps  ','massLF/kg ','empty ' &
 , 'FLmax/feet ', 'PMALTmax/feet ' &
  ,'flownairdist/m ','flown_Gdistance/m ' 
  
    
 
   print*,' open tried','F:\FLLISTSfinal\'//'outmassS_'//cdate//'.txt'
  ...
  OPen(5,file='F:\FLLISTS\'//cdate,action='read',form='unformatted') 
  print*,' opened','F:\FLLISTS\'//cdate !!!'EU/FLC'//cdate
...
!        if( 248755068 .ne. FLLISTS%flight_id) goto 111
!        istop=1
...
!      end do
      ...
      if(LPR) Print*,'  NW.le. (2+NWOLD)/2),nw,nwol,nfl',nw,nwold,nfl
     if(NW.lt.10) then 
      Print*,' Nw.lt.10',NW
       ngoto888=ngoto888+1
    print*,' goto 888 7'
...
!     call disttest2(NW)
 ...
 888 continue
   ...
   if(LPR) PRint*,'IPS,jairline,ibestm(Ips,Jairline),massresult6(6)', &
   IPS,jairline,ibestm(Ips,Jairline),massresult6(6)
...
! also computes flightairdist
     ...
     write(50, &
  '(I10,1x,a4,1x,I10,2i3,1x,7I2,6f10.0,f10.1,I10,6F10.0,7F12.5)') & 
    FLLISTS%flight_id,PICAO(IPS),is,IPS,jairline,imassvalid (1:6)&
    ,ibestm(Ips,Jairline) &
    ,massresult6(1:6),clrout,nclrout,massxfromLF(IS)  &
     ,0.,flmax,100.*PMalthft(IPS) &
     ,flightairdist,FLLISTS%flown_distance*1852.  
     ...
     NWR50=NWR50+1
     write(cresult,*) FLLISTS%flight_id,',',massresult6(6)
     write(51,*) trim(cresult)
 
 !???         if(flightairdist.lt.0.1*FLLISTS%flown_distance*1852.) stop
...
!!! ??? flight_id added 18 10 2024
  ...
  999 continue
   close(5)
   Print*,' all read done,IDAY,NFL,NWTOT=',IDAY,NFL,NWTOT  
!  ,' FLIGHT(1)%flight_id',FLIGHT(1)%flight_id
...
!!!!???   if(Nwr50.gt.1000) stop
  ...
  ,' FLIGHT(1)%flight_id',FLIGHT(1)%flight_id,' Nwr50=',Nwr50
   ...
   print*,' submissionfile finished,see'
   close(51)
     close(49)
     close(20)
...
!      end if
      ...
      if(FLIGHT(I)%aLtitude.lt.-600.) then
      ...
      open(11,file='test5_checkmonoton.txt')
      do I=1,NW
      ...
      Print*,' idtmaxmax=',idtmaxmax,' nwmax=',nwmax
         open(10,file='test5_check_IDT.txt')
...
!        end do
        ...
        if(I1.ne.NW) then
        if(LPR)  print*,' end parts elimiated',i1,NWOLD
        end if
        NW=I1
...
!      write(45,*) PICAO(IPS),POEM(IPS),PMdes(IPS),PSwingarea(IPS),clo(IPS)  &

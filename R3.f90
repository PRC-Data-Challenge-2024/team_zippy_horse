! R3 performs a detailed trajectory analysis
! derives mass estimates, for methods 1, 2,3, and 5 (based on the PS model) 
! and computes the necessary input for estimating the mass from the initial climb rate (method 4)
! makes use of the PS model from Poll and Schumann
...
MODULE mo_directories ! many code details
  USE mo_types,only: TFLIGHT,TFLIGHTLISTc,TFLIGHTadd,Tclimbdata
  IMPLICIT NONE
      Type(TFLIGHT),DIMENSION(:),ALLOCATABLE      :: FLIGHT ! data
      TYPE(TFLIGHTLISTc) :: FLLISTC 
...
!https://www.flugzeuginfo.net/acdata_php/acdata_7772_en.php
  ...
  ,Jairline,NPS,ic  &
  ,climbdataca  &
  ,lpr,nairlinex,massresult5,clrout,nclrout,nfcmax,flightairdist
  USE mo_types,only: TFLIGHTLISTC
  IMPLICIT NONE
  INCLUDE "CPARA.inc"
  INCLUDE "psbeta3.inc"
  INTEGER istat,NFL,NW,I,J  &
  ,ngoto888,NFLstep,NFLstepmax,itlastmass
  INTEGER*8 NWTOT
  INTEGER NFOUNDC,JDAYJ0,IDAY
 ...
 !      print*,' JDAYJ0',JDAYJ0
  ...
  Lflightready=.false.
  open(77,file='flighread',form='unformatted')
  write(77) Lflightready
  close(77)
  
           OPEN(20,File='PMTOM_OEM_outofrange.txt')
        write(20,*) 'flight_id ',' test ',' ATYP ',' IPS ',' value ','(IPS) ','number '
 
 
 open(49,file='oemerrors.txt')
      write(49,*) 'icao ' ,'IPS ','jairline ' &
      , 'itime_sincestart/s ' ,'duration/s ','flown_distance/nml ' &
            , 'TOW/kg ', 'imass ',' mass/kg ', 'OEM/kg ','MTOM/kg ','MZFM/kg ','flight_id '

            
 LOOPday: do iday=1,365! 271,365!365!262!262!262!1,2!5!1,1!1,1!2
 
 open(77,file='flighread',form='unformatted',action='read')
  ...
  open(50,file='F:\FLLISTCoutnew\'//'outmass_'//cdate//'.txt')
       write(50,*) 'flight_id ','ICAO ','ic ','IPS ','Jairline ' &
 ,'imassvalid1 ','imassvalid2 ','imassvalid3 ','imassvalid4 ','imassvalid5 ' &
 ,'massresult1/kg ','massresult2/kg ','massresult3/kg ','massresult4/kg ','massresult5/kg ' & 
 ,'clr/(m/s) ','nclrsteps  ' ,' given_TOW/kg ' &
 ,'err1/kg ','err2/kg ','err3/kg ','err4/kg ','err5/kg ' &
  , ' FLmax/feet ', ' PMALTmax/feet ','flownairdist/m ','flown_Gdistance/m '
   print*,' open tried','F:\FLLISTC\'//cdate !!!'EU/FLC'//cdate
  ...
  dowhileilast: do WHILE (NFLstep.lt.NFLstepmax) 
   111  continue
        ...
        windspeedmax=0.
            ...
            read(5,END=999)FLLISTC,NW,ic,Jairline
            nairlinemax=max(nairlinemax,Jairline)
       I2day=0
            imassvalid=0
            massresult5=0.
            massresult5(1)=massxfromLF(IC)
            
      
            
 
        if(LPR) print*,' NW,ic,Jairline ',NW,ic,Jairline
        if(LPR) print*,'FLLISTC ',FLLISTC    
        
        I=1
        if(I.gt.0 .and. lpr ) then
        
       Print*,'flight_id ',FLLISTC%flight_id
    Print*,' cdatex ',FLLISTC%cdatex
    
     Print*,' callsign ',FLLISTC%callsign
   
     Print*,' adep ',FLLISTC%adep
    
     Print*,' name_adep ',FLLISTC%name_adep
  
      Print*,'  country_code_adep ',FLLISTC%country_code_adep
   
     Print*,'  ades ',FLLISTC%ades
  
     Print*,'  name_ades ',FLLISTC%name_ades
 
     Print*,'country_code_ades ',FLLISTC%country_code_ades
   
     Print*,'  actual_offblock_time ',FLLISTC%actual_offblock_time
 
     Print*,' arrival_time/s ',FLLISTC%arrival_time
    
     Print*,'  aircraft_type ',FLLISTC%aircraft_type
    
     Print*,'  wtc ',FLLISTC%wtc
   
     Print*,'  airline ',FLLISTC%airline
     
 
     Print*,'  flight_duration/min ',FLLISTC%flight_duration
     Print*,'  taxiout_time/min ',FLLISTC%taxiout_time
     Print*,'  flown_distance/nml ',FLLISTC%flown_distance
    
      Print*,'  tow/kg ',FLLISTC%tow
 
     Print*,'  xdep ',FLLISTC%xdep
     Print*,'  ydep ',FLLISTC%ydep
     Print*,'  zdep/feet ',FLLISTC%zdep
     Print*,'  xdes ',FLLISTC%xdes
     Print*,'  ydes ',FLLISTC%ydes
     Print*,'  zdes(feet ',FLLISTC%zdes
 
      Print*,'  itwdep/s ',FLLISTC%itwdep  
      Print*,'  itwdes/s ',FLLISTC%itwdes  
      Print*,'  IFLinitialcruise/feet ',FLLISTC%IFLinitialcruise  
      Print*,'  IFLmax/feet ',FLLISTC%IFLmax  
...
!        if(255406203 .ne. FLLISTC%flight_id) goto 111
!        istop=1
...
!      if(NFL.gt.10) goto 111
 ...
 !!     
      ...
      I=0
     ifigt0pr: if(I.gt.0) then      
     print*,' FLLISTC%itwdep-iutc0,FLLISTC%xdep,ydep'  &
     ,FLLISTC%itwdep-iutc0,FLLISTC%xdep,FLLISTC%ydep
         do I=1,max(1,NW/10)
     ...
     do I=max(1,NW/10)+1,(NW*9)/10,max(1,(8*NW)/10000)
      ...
      ,FLIGHT(I)%altitude,FLIGHT(I)%groundspeed,FLIGHT(I)%vertical_rate 
                ...
                print*,' FLLISTC%itwdes-iutc0,FLLISTC%xdes,ydes'  &
     ,FLLISTC%itwdes-iutc0,FLLISTC%xdes,FLLISTC%ydes
      Print*
...
!
    ...
    print*,'  vor LICgt0,nfl',nfl
    
           call ACNUMBPSbeta(FLLISTC%aircraft_type,IPS) ! find IPS number for given aircrat-type
        ...
        if(FLLISTC%TOW.gt. 1.01*PMTOM(IPS))then
  write(20,*)FLIGHT(1)%flight_id,' MTOM  ',FLLISTC%aircraft_type,' ',IPS,FLLISTC%TOW,PMTOM(IPS)
         end if
        if(FLLISTC%TOW.lt. POEM(IPS))then
  write(20,*)FLIGHT(1)%flight_id,' OEM   ',FLLISTC%aircraft_type,' ',IPS,FLLISTC%TOW,POEM(IPS)
          print*, 'TOW<oem,ID=',FLIGHT(1)%flight_id,' OEM   ',FLLISTC%aircraft_type,' ',IPS,FLLISTC%TOW,POEM(IPS)
           end if
   ...
   NFOUNDC=NFOUNDC+1
    ...
    Print*,' before correctend  Nw,nfl',NW,nfl  
   if(LPR) print*,'FLLISTC%flight_id',FLLISTC%flight_id,' NFL',NFL
...
!      end if
!      end do
      if(LPR) print*,'end CHECK FOR LIMITS'
      if(LPR) print*,' FLLISTC',FLLISTC
      if(LPR) print*,' NFL',NFL
...
!    end do
   ...
   if(LPR) print*,' after smooth,NW,FLLISTC%flight_id,nfl',NW,FLLISTC%flight_id,nfl
...
!      call addmass(NW)
       RT=FLLISTC%flown_distance*1820
       if(LPR) print*,' rt',rt
       lnewoem=.true.
       FLoad=meanLFd(IDAY)
       ...
       Mass(2)=FLLISTC%TOW
       Mass(3)=PMTOM(IPS)
       if(LPR) print*,' IPS,',IPS,' ',PICAO(IPS),' ',RT,FLLISTC%flown_distance
       if(LPR) print*,' mass',mass,'FLLISTC%flight_id',FLLISTC%flight_id
    ...
    dvdttemp(I)=drittel*(FLIGHTadd(max(1,I-1))%dgsdtW+FLIGHTadd(I)%dgsdtW+FLIGHTadd(min(NW,I+1))%dgsdtW)
     ...
     write(20,*) FLIGHT(1)%flight_id,' MMO   ',FLLISTC%aircraft_type,' ',IPS,machmax,PMMO(IPS),Imachhigh
     end if
     if(IFLhigh.gt.0) then 
     write(20,*) FLIGHT(1)%flight_id,' FLmax ',FLLISTC%aircraft_type,' ',IPS,flmax,100.*PMalthft(IPS),IFLhigh
     end if
     
!     call eliminate_FL(NW)
     Imassclfound=0
     massxCL=0.5*(PMTOM(IPS)+POEM(IPS))
     massxCT=massxCL
...
!!!! massCT is used for method 5 
         ...
         ifm2: if(I.lt.NW/4) then
          ifm3: if(Imassclfound.le.3) then
           ifm4: if(flightadd(I)%Ioriginal.eq.1  .and. flightadd(I)%Invalid.eq.0  .and. dhdt.gt.10.) then
            ifm5: if(POEM(IPS).lt.massCL.and. massCL.lt.PMTOM(IPS)) then
        imassvalid(2)=1
        Imassclfound=Imassclfound+1
        massxCL=MIN(massCL,massxCL)
        massresult5(2)=massxCL
  ...
  ltfm1: if(Imass.eq.3) then
   if(flight(I)%altitude.gt.flmax-100.) then
    if(flight(I)%altitude.gt.0.25*100.*PMalthft(IPS).and.dhdt.ge.0.) then
     if(flightadd(I)%Ioriginal.eq.1 .and. flightadd(I)%Invalid.eq.0) then
      if(1.01*POEM(IPS).lt.massCT.and. massCT.lt.PMTOM(IPS)*0.99) then
        imassvalid(5)=1
         massxCT=min(massCT,massxCT)
          massresult5(5)=massxCT
      end if
     end if
    end if
   end if
       ...
       itlastmass=FLIGHTadd(I)%itw
           ...
           if(imass.ge.2  &
            .and. mass(IMass).lt.POEM(IPS)) then
             if(IWR49.eq.0) then
           print*,'I=',I,'imass,mass.lt.OEM,mass,TOW,imass,mass=',imass,mass(IMass)  &
            ,'OEM=',POEM(IPS),'TOW=',FLLISTC%TOW  &
            ,' ',FLLISTC%aircraft_type,' ',FLLISTC%flight_id  &
            ,'dist/km=',FLLISTC%flown_distance*1.830
            
            write(49,*) picao(IPS),' ',IPS,jairline &
            ,flightadd(I)%ITW-flightadd(1)%ITW &
            ,flightadd(NW)%ITW-flightadd(1)%ITW &
            ,FLLISTC%flight_duration*60 &
            , FLLISTC%flown_distance &
           ,FLLISTC%TOW &
           , IMass &
           ,mass(IMass) &
           ,POEM(IPS) &
   ...
   if(imassvalid(5).gt.0) then
    ...
    if(LPR) print*,' POEM(IPS)',POEM(IPS),' ',FLLISTC%aircraft_type,' IPS=',IPS,' NFL=',NFL
 
   call M3_SMassfromDO(NW,IPS)
  
   
   call M4_climbdata(NW,IC,IPS)
 
       
   if(LWRITE17) then
   write(17,*)FLLISTC,NW
!     call disttest2(NW)
    Ifl1=FLLISTC%IFLinitialcruise
    Ifl2=FLLISTC%IFLmax
    I=1
   ...
   if(LPR) print*
    Print*,'iday',iday,' NFOUNDC',NFOUNDC  &
    ,' NFL=',NFL,'  ngoto888',ngoto888
    print*,'############################################'
...
!     end if
 ...
 888 continue
    flightairdist=FLLISTC%flown_distance*1852  
   call M1_MassfromLF(NW,IC,IPS,IDAY)
! computes massresult5(1)  and flightadd(NW)%massfrom(1), 
! based on load factor and flown air distance
! also computes flightairdist
  
     write(50, &
     '(I10,1x,a4,1x,I10,2i3,1x,5I2,5f10.0,f10.1,I10,F10.0,5f10.5,4F10.0)') &     
    FLLISTC%flight_id,PICAO(IPS),ic,IPS,jairline,imassvalid&
    ,massresult5,clrout,nclrout,FLLISTC%TOW &
     ,(massresult5-FLLISTC%TOW)/FLLISTC%TOW,flmax,100.*PMalthft(IPS) &
     ,flightairdist,FLLISTC%flown_distance*1852.  
!!! ??? flight_id added 18 10 2024
     Lflightready(IC)=.true.
     end do dowhileilast
  999 continue
   close(5)
  ...
  ,' FLIGHT(1)%flight_id',FLIGHT(1)%flight_id
   ...
   open(51,file='climbdatanew'//cdate,form='unformatted')
   write(51)climbdataca
   close(51)
  ...
  close(77)
   ...
   print*,'number of  missing flights, J=', J
    ...
    subroutine M1_MassfromLF(NW,IC,IPS,IDAY) ! -> massresult5(1) and flightadd(NW)%massfrom(1)
           USE mo_directories, ONLY: FLIGHT, flightadd ,massxfromLF,meanLFd &
           ,imassvalid,lpr,massresult5
  IMPLICIT NONE
  INTEGER,INTENT(IN):: NW,IC,IPS,IDAY
  REAL massresult
        if(LPR) print*,' nw,ips,iday', nw,ips,iday
        massresult=massxfromLF(IC)
            if(Nw.gt.100) then
         call M1_LOADF_wind(NW,IPS,IDAY,massresult)
          flightadd(NW)%massfrom(1)=massresult ! lfw
         end if
        imassvalid(1)=1
        massresult5(1)=massresult   
         if(NW.gt.0) flightadd(NW)%massfrom(1)=massresult         
        if( massresult.le.100.) then
        print*,'M1_MassfromLF: massresult.eq. 100.) ',massresult
        stop
        end if
      ...
      subroutine OUTFLC(NW,IPS,cfile)
           USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,IUTC0,NPLOT,lpr,nps
           IMPLICIT NONE
...
!           REAL z0 ! test of integral vertical velocity compared to altitude -> vertical rate comes in m/s
 ...
 !        end do
    ...
    ,flightadd(Ifl1)%ITW-IUTC0,FLIGHT(Ifl1)%altitude  &
    ,flightadd(Ifl2)%ITW-IUTC0,FLIGHT(Ifl2)%altitude  &
    , ' ',PICAO(IPS),' ',FLLISTC%flown_distance 
    else
       write(10,*) FLIGHT(I),flightadd(I),flightadd(I)%ITW-IUTC0 
    end if
...
! sorts waypoints by times ITW
   ...
   USE mo_directories,ONLY: FLIGHT,FLIGHTadd,FLLISTC,iutc0,lpr
      IMPLICIT NONE
    ...
    Brr(I)=J
     ...
     if(LPR)then
      if(I.le.10) then
       ...
       print*,'FLLISTC%flight_id',FLLISTC%flight_id
      end if
     end if
       NW=I
    if(NW.gt.2) then
...
! purpose: ssorttime orders the flight data such that the time is monotonically increasing
      USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,NWX,lpr
      IMPLICIT NONE
      INTEGER,INTENT(INout)::NW
...
! inserts  waypoints between IW1 and IW2
      USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,IUTC0,flmin
...
!        stop
  ...
  subroutine testdist
     ...
     print*,' end of testdist'
!    stop
  END SUBROUTINE  testdist
  
  
              
      subroutine ADDMASS(NW)
           USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,IUTC0,NPLOT,lpr
           IMPLICIT NONE
           INTEGER,INTENT(IN)::NW
           character*1,Parameter:: CorS='C'
           INTEGER I
           character*9 cd
!           REAL z0 ! test of integral vertical velocity compared to altitude -> vertical rate comes in m/s
            if(NPLOT.lt.2) THEN
           write(cd,'(I9)')FLIGHT(1)%flight_id
           open(10,file='FL'//CorS//cd//'.txt')
                    write(10,'(A1,1x,5a12,11a15,2(1x,a4,1x,a2),1x,a4,1x,a1,1x,a10,2A12,100A12)') 'O','I'  &
                    ,'flight_id','ITW','IUTC0','Ignore'  &
           ,'lon/deg','lat/deg','alt/ft'  &       
           ,'track','w/(m/s)','CLRW/(m/s)'  &
     ,'track_unwr','u_wind','v_wind'  &
     ,'temp/K','humidity'  &
     ,'adep','cp','ades'  &
     ,'cs','ICAO','w','distance','gr_sp/(m/s)','GSc/(m/s)'  &     
     ,' TASW','Machw'  &
     ,'dgsdtW','Segmw/m','cosaw','sinaw'
           I=0
           write(10,'(I1,1x,5I12,11F15.6,2(1x,a4,1x,a2),1x,a4,1x,a1,1x,I10,2F12.2)') FLIGHTadd(1)%Ioriginal,I  &
          
           ,FLIGHT(1)%flight_id,FLLISTC%ITWdep-IUTC0,IUTC0,0  &
           ,FLLISTC%xdep,FLLISTC%ydep,0.
!           z0=0
           do I=1,NW
!           z0=z0+FLIGHT(I)%vertical_rate
           write(10,'(I1,1x,5I12,11F15.6,2(1x,a4,1x,a2),1x,a4,1x,a1,1x,I10,2F12.2,100F12.4)')FLIGHTadd(I)%Ioriginal,I  &
           ,FLIGHT(I)%flight_id,FLIGHTadd(I)%ITW-IUTC0,IUTC0,FLIGHTadd(I)%INVALID  &
           ,FLIGHT(I)%longitude,FLIGHT(I)%latitude,FLIGHT(I)%altitude  &
          ,FLIGHT(I)%track,FLIGHT(I)%vertical_rate,FLIGHTadd(I)%CLRW &
     ,FLIGHT(I)%u_component_of_wind,FLIGHT(I)%v_component_of_wind  &
     ,FLIGHT(I)%temperature,FLIGHT(I)%specific_humidity  &
     ,FLLISTC%adep,FLLISTC%country_code_adep  &
     ,FLLISTC%ades,FLLISTC%country_code_ades  &
     ,FLLISTC%aircraft_type  &
     ,FLLISTC%wtc,FLLISTC%flown_distance*1852,FLIGHT(I)%groundspeed,flightadd(I)%GSDISTW  &
     ,FLIGHTadd(I)%TASW,Flightadd(I)%Machw  &
     ,FLIGHTadd(I)%dgsdtW,flightadd(I)%SEGMW,FLIGHTadd(I)%cosaw,FLIGHTadd(I)%sinaw 
!           print*,' I,NW,z0,FLIGHT(I)%altitude ',I,NW,z0/0.3048,FLIGHT(I)%altitude 
           end do
           I=NW+1
           write(10,'(I1,1x,5I12,11F15.6,2(1x,a4,1x,a2),1x,a4,1x,a1,1x,I10,2F12.2)') FLIGHTadd(NW)%Ioriginal,I  &
           ,FLIGHT(NW)%flight_id,FLLISTC%ITWdes-IUTC0,IUTC0,0  &
           ,FLLISTC%xdes,FLLISTC%ydes,0.
                  NPLOT=NPLOT+1
           if(LPR) print*,'ADDMASS(NW)',NW,' id=',CorS,FLIGHT(1)%flight_id,' NPLOT=',NPLOT
    
           if(NPLOT.ge.30) then
          Print*,' stop in ADDMASS bcacause of more than 30 plots'
             stop
             end if   
       print*,' plot for FLIGHT(1)%flight_id done ',CorS,FLIGHT(1)%flight_id   
        else
         if(LPR) print*,' ADDMASS  deactivated because of too large NPLOT',NPLOT   
      end if        
      close(10)
...
!          print*,' FLIGHTadd(NNew)%ITw-iutc0',FLIGHTadd(NNew)%ITw-iutc0
    ...
    END SUBROUTINE  smoothdvdt
       ...
       USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,Nsmooth,lpr
      IMPLICIT NONE
      INTEGER,INTENT(IN)::NW
   ...
   FLLISTC%IFLinitialcruise=1
    if(FLLISTC%IFLMAX.gt.1) then
 
     JSIC=0
!     print*,' FLLISTC%IFLMAX,min(FLLISTC%IFLMAX,NW/4),1+Istart'  &
!     ,FLLISTC%IFLMAX,min(FLLISTC%IFLMAX,NW/2),1+Istart
  
    DOI: do I=min(FLLISTC%IFLMAX,NW/2),1+Istart,-1
    if(FLIGHT(I)%altitude.lt.FLIGHT(FLLISTC%IFLMAX)%altitude-800.  &
...
!    print*,'JSIC',JSIC
    ...
    JSIC=JSIC+1
!    print*,'JSIC',JSIC
    if(jsic.gt.30/nsmooth) FLLISTC%IFLinitialcruise=J
    end if
    end if
    end if
    end do doj
...
!    write(21,*) FLLISTC%flight_id,' ',FLLISTC%aircraft_type,' '  &
!    ,FLLISTC%IFLinitialcruise,FLIGHT(FLLISTC%IFLinitialcruise)%altitude  & 
!    ,FLLISTC%IFLmax,FLIGHT(FLLISTC%IFLmax)%altitude  &
!    ,(FLLISTC%flown_distance*1.830)
!    print*,FLLISTC%flight_id,' ',FLLISTC%aircraft_type,' '  &
!    ,FLLISTC%IFLinitialcruise,FLIGHT(FLLISTC%IFLinitialcruise)%altitude  & 
!    ,FLLISTC%IFLmax,FLIGHT(FLLISTC%IFLmax)%altitude  &
!    ,(FLLISTC%flown_distance*1.830)
...
! corrects for unrealistic flight levels at start or departure
      USE mo_directories,ONLY: FLIGHT,FLIGHTadd,FLLISTC,flUAS,lpr
      IMPLICIT NONE
       INTEGER,INTENT(INout)::NW
      ...
      nwfraction=max(nwfraction,1)
        ...
        if(LPR) print*,' FLLISTC%xdep,FLLISTC%ydep',FLLISTC%xdep,FLLISTC%ydep

        if(LPR) print*,' flmin at departure',flmin,' fluas',fluas
        
        ifflmindep: IF(flmin.gt.fluas+FLLISTC%zdep) then
        NW=NW+1
        do I=NW,2,-1
        ...
        I=1
        FLIGHT(I)=FLIGHT(I+1)
        FLIGHT(I)%timestamp=FLLISTC%actual_offblock_time
        FLIGHT(I)%timestamp2=FLLISTC%actual_offblock_time(12:20)
        FLIGHT(I)%longitude=FLLISTC%xdep-0.01
        FLIGHT(I)%latitude=FLLISTC%ydep-0.01
        FLIGHT(I)%altitude=FLLISTC%zdep
        FLIGHTadd(I)=FLIGHTadd(I+1)
         FLIGHTadd(I)%ITW=FLLISTC%itwdep
         FLIGHTadd(I)%ioriginal=0
         FLIGHTadd(I)%INVALID=0
...
!        end do
         if(LPR) print*,'correctend: FLLISTC%xdes,FLLISTC%ydes',FLLISTC%xdes,FLLISTC%ydes
        if(LPR) print*,' flmin,fluas',flmin,fluas
        ifflmin: IF(flmin.gt.fluas+FLLISTC%zdes) then
        NW=NW+1
        I=NW
        FLIGHT(I)=FLIGHT(I-1)
        FLIGHT(I)%timestamp=FLLISTC%arrival_time
        FLIGHT(I)%timestamp2= FLLISTC%arrival_time(12:20)
        FLIGHT(I)%longitude=FLLISTC%xdes+0.01
        FLIGHT(I)%latitude=FLLISTC%ydes+0.01
        FLIGHT(I)%altitude=FLLISTC%zdes
         FLIGHTadd(I)=FLIGHTadd(I-1)
         FLIGHTadd(I)%ITW=FLLISTC%itwdes
         FLIGHTadd(I)%ioriginal=0
         FLIGHTadd(I)%INVALID=0
         FLIGHTadd(I)%segmW=100.  
         FLIGHT(I)%specific_humidity=1.e-5         
...
!       .or.FLIGHT(2)%altitude.gt.flUAS) then
 ...
 ! includes airport coordinates at beginning and end of trajectory
           USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,lpr
           use mo_cocip_parameters,only: UNITY,PIR180
           IMPLICIT NONE
           INTEGER,INTENT(InOUT)::NW
       ...
       I2=0
        ...
        flight(1)=flight(2)
        flight(1)%longitude=FLLISTC%xdep
        flight(1)%latitude=FLLISTC%ydep
        flight(1)%altitude=FLLISTC%zdep
        flight(1)%groundspeed=0.
        flight(1)%vertical_rate=0.
         print*,'test: FLLISTC%ITWdep,FLIGHTadd(1)%ITW-',FLLISTC%ITWdep,FLIGHTadd(1)%ITW
       
         FLIGHTadd(1)%ITW=MIN(FLLISTC%ITWdep,FLIGHTadd(1)%ITW-1)
       ...
       ,flight(1)%altitude,flight(2)%altitude
        ...
        flight(NW)%longitude=FLLISTC%xdes
        flight(NW)%latitude=FLLISTC%ydes
        flight(NW)%altitude=FLLISTC%zdes
        flight(NW)%groundspeed=0.
        flight(NW)%vertical_rate=0.
        FLIGHTadd(NW)%ITW=FLLISTC%ITWdes+60
       ...
       if(FLIGHTadd(1)%itw.lt.FLLISTC%itwdep)  then
        do I=1,NW
         if(FLIGHTadd(I)%itw.ge.FLLISTC%itwdep) then
        i1=I
        exit
        end if
        end do
        ...
        I2=NW 
        itdes=FLLISTC%itwdes     
    
        if(FLIGHTadd(NW)%itw.gt.FLLISTC%itwdes)  then
        if(LPR) print*,' FLIGHTadd(NW)%itw.gt.FLLISTC%itwdes',FLIGHTadd(NW)%itw,FLLISTC%itwdes
        do I= NW,I1,-1
        if(FLIGHTadd(I)%itw.le.FLLISTC%itwdes) then
        i2=I
        exit
        end if
        end do
...
!        end do
        ...
        print*,' eliminate_zeroalt,nwold,nw',nwold,nw
        I1=NW
        NWOLD=NW
        ...
        if(I1.ne.NW) then
        print*,' end partelimiated',i1,NWOLD
        end if
        NW=I1
...
!       print*,'I,jmax,dz,dzlimit',I,jmax,dz,(1+Jmax-I)*dzlimit
    ...
    END SUBROUTINE  disttest2
      ...
      USE mo_directories,ONLY: FLIGHT,FLLISTC,lpr,NPS
...
! as proposed by Ian Poll:A method for the estimation of the take-off mass
      ...
      print*,' IPS,Rt,FLoad',IPS,Rt,FLoad
      stop
      end if
      ...
      floadfit=(FLLISTC%TOW*fexp-POEM(IPS))/(PMZFM(IPS)-POEM(IPS))   
...
! MassfromDO from design optimum (Ian Poll,2.10.2024),see SMassfromMach
       USE mo_directories,ONLY: FLIGHT,FLLISTC,FLIGHTadd,Nsmooth  &
       ,ic,lpr,nairlinex,jairline  &
       ,NPS,massresult5,imassvalid
      ...
      real PICAOPA,CY17,Cz17,p,x1,x2,flmax,riflmax,massCLO
       INTEGER I,nwchecked,iflmax,itime,nsum,NWV,istart,J,Imethod
       INTEGER ITV(nw)
   ...
   ifl2gt3: if(FLLISTC%IFLmax.gt.1.and. FLLISTC%flown_distance.gt. 200) then
   nwv=0
   do I=1,NW
   if (FLIGHTadd(I)%ioriginal.eq.1.and. FLIGHTadd(I)%invalid.eq.0) then
   NWV=NWV+1
   ...
   exit
      ...
      itime=0
       ...
       FLIGHTadd(1:NW)%massfrom(3)=Flightadd(1)%massw(3) ! CLO
   ...
   flhigh: if (flmax.gt.(0.84*100.)*PMalthft(IPS)) then
      ...
      massCLO=CZ17*p*(FLIGHTadd(I)%machw**2)*(PSwingarea(IPS)*clo(IPS)*0.7/g)
        ...
        if(massCLO.lt.flightadd(I)%massw(3)) then
  
        if(massCLO.gt.flightadd(I)%massw(1)) then
        tfmsum=flightadd(1)%massw(2)-flightadd(I)%massw(2)
        FLIGHTadd(I)%massfrom(3)= massCLO+tfmsum ! CLO
        massCLO=massCLO+tfmsum
       
 !           print*,' I,massCLO=',I,massclo,flightadd(I)%massw(3),flightadd(I)%massw(1) 
...
!      write(45,*) PICAO(IPS),FLLISTC%TOW,POEM(IPS),PMdes(IPS),PSwingarea(IPS),clo(IPS)  &
!       ,FLLISTC%flight_duration,FLLISTC%taxiout_time,FLLISTC%flown_distance ,itime/60.  &
!      ,FLIGHT(I)%altitude,p,FLIGHT(I)%temperature,FLIGHTadd(I)%machw  &
!      ,flightadd(1)%massw(1),flightadd(1)%massw(3),flightadd(I)%massw(1),flightadd(I)%massw(3) &
!      ,massCLO,((massCLO-flightadd(1)%massw(2))/flightadd(1)%massw(2))
      
      
        if(LPR) print*,' massCLO',massCLO,flightadd(1)%massw(1) ,flightadd(1)%massw(3)
       if(massCLO.gt.flightadd(1)%massw(1)  &
       .and. massCLO.lt.flightadd(1)%massw(3))   THen
         imassvalid(3)=1
         massresult5(3)=massCLO
      ...
      subroutine M1_LOADF_wind(NW,IPS,IDAY,TOM)
        ...
        ,flight,imassvalid,flightairdist,FLLISTC
...
!       print*,' NW',NW,'flight(1)%flight_id',flight(1)%flight_id
       I=1
       idt=flightadd(I)%ITW-FLLISTC%itwdep
...
!       print*,' I,flightadd(I)%ITW,idt,Tas,rt',I,flightadd(I)%ITW,idt,Tas,rt
          ...
          I=NW
       idt=FLLISTC%itwdes-flightadd(I)%ITW
       TAS=flightadd(NW-1)%TASW
       RT=RT+idt*TAS    
!       print*,'FLLISTC%flown_distance*1820,RT',FLLISTC%flown_distance*1820, RT
!       I=1
!       print*, &
!       'xdep',flight(I)%longitude,FLLISTC%xdep
!       print*, &
!       'ydep',flight(I)%latitude,FLLISTC%ydep
!       print*, &
!       'ITWdes',flightadd(I)%itw-iutc0,FLLISTC%itwdep-iutc0
!       I=NW
!       print*, &
!       'xdes',flight(I)%longitude,FLLISTC%xdes
!       print*, &
 !      'ydes',flight(I)%latitude,FLLISTC%ydes
!       print*, &
!       'ITWdes',flightadd(I)%itw-iutc0,FLLISTC%itwdes-iutc0
!       stop
       flightairdist=RT
                     lnewoem=.true.
 ...
 !       Print*,'M1_LOADF_wind I,idt,rt,TAS,fexp,tom',I,idt,rt,TAS,fexp,tom
       ...
       subroutine M4_climbdata(NW,IC,IPS)
         USE mo_directories, ONLY: FLIGHT, flightadd,FLLISTC,IUTC0,meanLFd  &
         ,Jairline,clrmintyp,clrmaxtyp  &
         ,imassvalid,climbdataca,lpr,NPS,massresult5,clrout,nclrout
  USE mo_cocip_parameters, ONLY: LCVCC
  IMPLICIT NONE
  INTEGER,INTENT(IN):: NW,IC,IPS
  INCLUDE "CPARA.inc"
  INCLUDE "psbeta3.inc"
...
! preliminary solution
              ...
              FLIGHTadd(NW)%massfrom(4)=POEM(IPS)  &
   +(pmtom(IPS)-POEM(IPS))*(clr-clrmintyp(IPS))/(clrmaxtyp(IPS)-clrmintyp(IPS))
   
              imassvalid(4)=1
              massresult5(4)=FLIGHTadd(NW)%massfrom(4)
              print*,'clr*ntkmean',clr*ntkmean.gt.1000.,clr*ntkmean,clr,ntkmean
      clrgt5: if(clr*ntkmean.gt.1000.) then
      clrout=clr
      nclrout=ntkmean
           
     climbdataca(IPS,Jairline)%clrm=climbdataca(IPS,Jairline)%clrm+clr
      climbdataca(IPS,Jairline)%a12=climbdataca(IPS,Jairline)%a12+clr
      climbdataca(IPS,Jairline)%a22=climbdataca(IPS,Jairline)%a22+clr**2
      climbdataca(IPS,Jairline)%b1=climbdataca(IPS,Jairline)%b1+FLLISTC%tow
      climbdataca(IPS,Jairline)%b2=climbdataca(IPS,Jairline)%b2+FLLISTC%tow*clr
      climbdataca(IPS,Jairline)%nclr=climbdataca(IPS,Jairline)%nclr+1
    ...
    FLIGHTadd(NW)%massfrom(4)=min(flightadd(1)%massw(3),FLIGHTadd(NW)%massfrom(4))
    FLIGHTadd(NW)%massfrom(4)=max(flightadd(1)%massw(1),FLIGHTadd(NW)%massfrom(4))

...
!      open(49,file='airport_notfound.txt')
      ...
      latmax=-90.E8
      do I=1,N
      latmax=max(latmax,yairport(I))
      latmin=min(latmin,yairport(I))
      lonmax=max(lonmax,xairport(I))
      lonmin=min(lonmin,xairport(I))
      yairport(I)=max(-90.,yairport(I))
      yairport(I)=min(90.,yairport(I))
      xairport(I)=max(-180.,xairport(I))
      xairport(I)=min(180.,xairport(I))
      end do

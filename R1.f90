...
!  together with an airport information subroutine.
!
!
...
! (based on a data set for ICAO airport identifier with longitude, latitude and altitude) provided by Roger Teoh

! Numbers the airlines (callsigns could also be used),
...
!
!  This code is compiled and executed by
!
...
!      ,ydep(I),xdes(I),ydes(I))
      ...
      ,FLLISTC(I)%ydep,FLLISTC(I)%zdep,FLLISTC(I)%xdes,FLLISTC(I)%ydes,FLLISTC(I)%zdes)
       if(Abs(FLLISTC(I)%ydep).gt.90. .or. Abs(FLLISTC(I)%ydes).gt.90.)     then
       print*,'I,adep,ades,xdep,ydep,zdep,xdes,FLLISTC(I)%ydes,FLLISTC(I)%zde', &
      FLLISTC(I)%adep,FLLISTC(I)%ades,FLLISTC(I)%xdep &
      ,FLLISTC(I)%ydep,FLLISTC(I)%zdep,FLLISTC(I)%xdes,FLLISTC(I)%ydes,FLLISTC(I)%zdes
      stop
      end if
 ...
 !    if(FLLISTC(I)%itwdes-FLLISTC(I)%itwdep.le.0) stop
...
!      USE mo_directories, ONLY: IUTC0
 ...
 !     ,FLLISTS(I)%tow    
  ...
  999 continue
      ...
      ,FLLISTS(I)%ydep,FLLISTS(I)%zdep,FLLISTS(I)%xdes,FLLISTS(I)%ydes,FLLISTS(I)%zdes)
       end do
!       print*,' findairportdepdes done'
